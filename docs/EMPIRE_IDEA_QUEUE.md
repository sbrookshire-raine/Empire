...
| E-23 | **Error Book is inert in the autonomous path (nothing appends, nothing hints)** | `ready` | The routing index said "misses go to the Error Book", but in the autonomous path (the norm since 2026-09-23) **no code writes or reads it**: `error_book_append` / `format_error_book_hint` are called only from `frontend/wiki_drift_api.py`, i.e. the legacy `EMPIRE_WIKI_MIDDLEWARE=1` escape hatch, and there is no agent-facing append tool (`wiki_scratch_upsert` writes the *scratchpad*, not the book). Evidence: today's whole trace = 52× `wiki_scout_search`, 38× `wiki_read_section`, **0** error-book entries. | **Proposed fix (deterministic, not prompt compliance):** have `wiki_scout_search` append the miss server-side when it returns `ok=false` and surface the existing hint in the payload, so a repeated dead end is flagged back to Eve without her remembering to log it. Then the prompt line can stay short |
| E-24 | **Trace `chars` metric overcounts ~n² (deltas look cumulative)** | `ready` | `message.appended` records sum to far more than the answer: turn `84db2ac89855` = **289 records, sum 209,011 chars, largest single chunk 1,451** — the n² pattern of cumulative deltas (289 × ~723 ≈ 209k), so a ~1.5k-char answer reads as 209k in the trace and misleads any latency/size read. | Fix in `frontend/serve.py`'s relay: record the true delta length (or drop `chars`). Cosmetic — no behaviour impact |

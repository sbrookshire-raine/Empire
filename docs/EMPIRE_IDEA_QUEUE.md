...
| E-16 | **Small-model fluency on a multi-card payload (7B fails R-02's acceptance case)** | `ready` | Measured 2026-09-24 on `empire-fast:7b` with `How do magnets work?`: the tool call is correct (`wiki_scout_search` 46.8 ms → `wiki_read_section` 232.0 ms) and the *model* is fast, but the **text step** fails two ways — (a) the whole reply was the call it never made, `wiki_read_section("magnetism", section="magnetic_fields_and_theory")`, and (b) two further attempts ran **301.6 s / 304.7 s**. The 14B presents the readings and asks in 18–38 s. So the blocker is not context, tool calling or routing (all now proven) but instruction-following on a 3-card payload. Options: (a) tighten the ambiguity rule for small models to a one-sentence shape ("name the two/three options and ask which" — no choosing); (b) send only the two most likely cards to a 7B; (c) accept 14B-only for ambiguous lookups and route small models to plain hits; (d) test whether a short "answer now in one sentence" suffix appended to the tool result fixes the leak. Note the harness now converts the leak into an honest failure, so this can no longer reach the speaker as gibberish. | Evidence: `eve-audit/r02-b-7b-graded.txt`, `eve-audit/r02-trace-summary.txt` |

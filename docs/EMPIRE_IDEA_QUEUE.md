...
| E-13 | **Title DNS polysemy: "magnets" → "The Magnets"** | `ready` | **Phase 1 of [`REFACTOR_PLAN.md`](REFACTOR_PLAN.md) (R-02) — the load-bearing slice.** `wiki_scout_search("magnets")` resolves via Title DNS to **The Magnets** (a group) with `usable: true` and no ambiguity signal, so a model that trusts the first card answers a physics question with a band. Deterministic candidate fix: when the matched title is `<lead-article-prefixed>` form of the user's bare common noun ("The Magnets" for asked "magnets"), set `ambiguous: true` + a rule *"the matched title carries a leading 'The' you did not ask for — check whether the user means that specific entity before answering"* (disambiguation already exists in `pipeline/wiki_title_dns.py`). Re-run `scripts/ab-fast-toolcalling.py` + the browser A/B after the change. | This is what let the 7B fail; the 14B only survived it by searching more. Evidence: `eve-audit/ab-a-14b.txt`, `eve-audit/ab-b-7b.txt` |

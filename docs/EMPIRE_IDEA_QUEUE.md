...
| E-19 | **Media limb: YouTube links/resources (and a player later)** | `idea` | Architect asked 2026-09-24 whether a connected YouTube API could let Eve grab links/resources, and whether an embedded player could open in a window later. **Feasible with existing slots — nothing new to invent:** (a) *Search/metadata/links* — YouTube Data API v3 `search.list` + `videos.list` + `playlists.list`/`playlistItems.list` return video/channel/playlist URLs, titles, descriptions (where people put their resource links), durations, thumbnails. Cost: **100 quota units per search** against the default 10,000/day → gate every call behind the limb and cache results like `web_scout` does under `04_Thought_Experiments/web_cache`. (b) *Transcripts* are **not** in the API for videos you do not own (`captions.download` is owner-scoped) — that path is local `yt-dlp` (+ whisper via Speaches, which already runs locally), with ToS/attribution caveats, so keep it a separate opt-in limb, not the same one. (c) *Player later*: the Workbench is zero-build HTMX/Alpine, so an embed is an `<iframe src="https://www.youtube.com/embed/<id>">` in a dock panel or a new `media.html` LEGO page — the CLARITY "products have their own page" slot. (d) *Storage*: the link library belongs in **PocketBase** (state: survives reboot, hand-editable, backed up), "which videos do I have about X" is Cognee (`propose_remember` → your confirm, never auto-promote). Wiring is the existing pattern: `pipeline/youtube_scout.py` + `mcp/` server or agent tool gated by a new Toolbelt category (`media_scout`, off by default, admitted by `admit_for_goal`), registered in `config/eve-capabilities/capability-registry.yaml` with `network: allow` for that one host, a doc in `config/eve-capabilities/tool-docs/`, and an entry in the parity/ceiling tests. **Identity tension to decide first:** EMPIRE's non-negotiables are *meter-free / zero-cloud*; a YouTube key is a metered external dependency (quota + key handling), so it must be opt-in and deny-by-default exactly like `web_scout` and Research Partner — or the media limb stays local-only (yt-dlp/metadata without the Data API). | Also note the honest limit for the Architect's "video that explains it" case: search can *find and link* a video; it cannot verify the video's quality or that it explains the topic — Eve should present it as a candidate, not as a verified answer |

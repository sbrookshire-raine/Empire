...
| E-25 | **Open-ended how-to questions still answer unlabelled, with no archive attempt** | `ready` | After the E-22 fixes went live, re-asking the drums question produced a normal reply (no more recital) but **0 tool calls and no provenance label** — turn `05fef50e26e8`: 15.8 s, `"Let's approach this creatively…"`, plus a mild routing narration ("let's find some basic drumming tutorials"). One sample is not proof either way, so the labelling behaviour is **not yet demonstrated**. | Two candidate levers, both deterministic: **(1)** near-miss candidates in the miss payload — `SELECT title_norm FROM pages WHERE title_norm LIKE 'drum%'` uses the primary-key index (fast), so a failed phrase query could hand back `Drum kit` / `Drum` / `Drum rudiment` and the model only has to pick one; **(2)** the frontend already knows whether any tool ran in the turn, so `serve.py` could annotate a zero-tool answer as general knowledge instead of trusting the model to label itself. Re-ask 2–3 times first to see if E-22's rule lands on its own |

...
| E-25 | **Thought-experiment / idea-transfer path: ledger reachable, primitives still not named by the model** | `ready` | **Built 2026-09-24** (the Architect's "100% thought experiment" workflow): **`primitive_lookup`** searches the decoded ledger (501 rows / 247 distinct; `thing → mechanism → primitives → sibling`) and hands back the ledger's own vocabulary on a miss; **`thought_experiment_read`** lists/reads past notes (capture was write-only); `skill-thought-experiments.md` now carries the six-step protocol (ground both terms → name the primitive → state the mapping → **test the testable part with `author_code`+`python_verify`** → label → capture with a falsifier); `empire-routing-detail.md` routes transfer asks; and `serve.py` injects an **AUTHORITATIVE PRIMITIVE LEDGER** block next to the ask (same pattern as the catalog block) because a prompt hint alone never fired the route — measured: 6 phrase-shaped wiki searches, **0** `primitive_lookup` calls. Live after the build: the block **is** injected (verified in the prompt now that the audit log records it) and her grounding improved — she walks the title ladder (`Juggling` → `Drumming` (snipe birds) → `DRUMS` (a spacecraft) → `drum kit` ✓) instead of reporting an empty archive. **Still open:** she does not name a primitive (`Timing & Sync`, `Sensory Feedback`) or label provenance; the leap remains the Architect's. Two matching bugs were found and fixed on the way: substring hits (`play` in "replayed") and common words (`one` in "one help button") — now word-boundary + a rarity filter (a term in >25% of rows is ignored), so an unmatched question honestly returns the vocabulary instead of irrelevant mechanisms. | Re-ask: expect the ledger block in the prompt (grep `eve-audit/active_chat.log` for `PRIMITIVE LEDGER`), archive-grounded leads for both titles, and — the open part — a named primitive. Tests: `tests/pipeline/test_primitive_lookup.py`, `tests/pipeline/test_thought_experiment.py`, `tests/frontend/test_transfer_context.py` (531 total) |

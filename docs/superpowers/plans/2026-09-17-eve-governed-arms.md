...
**Goal:** Give Eve reliable, governed, artifact-producing "arms" — not a new
stack, not third-party MCP servers. Each arm follows the existing EMPIRE pattern
already proven by docling/github-scout:
...
### Phase 2 — Artifact + verification arms
...
1. `create_spreadsheet` — openpyxl (no-auth, pure Python) writing `.xlsx` with
   ...
   external-link tests.
2. `python_verify` — uv + Ruff + pytest in a disposable worktree; no inherited
   secrets; diff+test report out.

- [ ] `pipeline/create_spreadsheet.py`, `pipeline/python_verify.py`
- [ ] `mcp/create_spreadsheet_mcp.py`
- [ ] `agents/empire-task-agent/agent/tools/create_spreadsheet.ts`, `python_verify.ts`
- [ ] `agents/empire-task-agent/agent/skills/skill-create-spreadsheet.md`, `skill-python-verify.md`
- [ ] `tests/pipeline/test_create_spreadsheet.py`, `test_python_verify.py`

**Gate:** `.xlsx` opens, formulas valid, injection blocked; worktree change
represented as diff, tests pass; no production push.
...
### Phase 4 — Public-web + code isolation (gated)
...
carries source/time/version. Held until Phase 1-3 are green.
...
- Playwright/browser, Whisper, FFmpeg, Serena, public datasets — Phase 4+, measured-exception only.

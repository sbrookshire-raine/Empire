# scenario-regret 0.1.0

Rank decisions using minimax regret. Original standard-library-only implementation for a local thought-experiment layer.
...
```sh
python3 -m venv .venv
.venv/bin/python -m pip install --no-index --no-deps dist/scenario_regret-0.1.0-py3-none-any.whl
.venv/bin/scenario-regret --help
.venv/bin/scenario-regret run < examples/input.json
.venv/bin/python -m unittest discover -s tests -v
```

Windows PowerShell (untested): `py -m venv .venv`, then `.venv\Scripts\python.exe -m pip install --no-index --no-deps dist/scenario_regret-0.1.0-py3-none-any.whl` and `Get-Content -Raw examples/input.json | .venv\Scripts\scenario-regret.exe run`.

## CLI
`scenario-regret run` reads one UTF-8 JSON object from stdin. `scenario-regret schema` prints its input schema. `scenario-regret mcp` starts the MCP server. `--help` and `--version` are supported. No positional file inputs or output writes. Exit 0: success; 2: input/configuration/usage error; 1: internal error; 3: broken output pipe. Output is deterministic sorted-key JSON. Structured input/expected output live in `examples/`.

## MCP registration
Edit `mcp/config.example.json`: replace the interpreter path with the absolute path to your installed environment. Windows uses `.venv\Scripts\python.exe`. The MCP client must support legacy `2025-11-25` stdio. Tool name: `scenario_regret`. Modern-only MCP clients are not supported. No client-specific EVE integration was tested.
...
Rebuild offline: `python scripts/build.py`. The original PEP 517 backend needs no build dependencies. Wheel bytes are reproducible for identical source inputs. Uninstall: `.venv/bin/python -m pip uninstall -y scenario-regret`. Deleting an environment is a user action, not performed by runtime code.

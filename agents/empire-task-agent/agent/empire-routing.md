...
| Local Wikipedia facts / multi-hop wiki work (who is X, cast, briefs, sections) | Prefer server-injected `[[EMPIRE_WIKI_LOOKUP]]` (Title DNS + sections). Use **`wiki_scratch_upsert`/`wiki_scratch_read`** for multi-hop. **`wiki_read_section`** when a named H2 is needed and no LOOKUP lock. **Do not** call `wiki_scout_search` when LOOKUP is present. **Do not** mention Weaviate/Docker/8091. Misses go to Error Book — do not invent |
